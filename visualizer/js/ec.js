(function( $ ) {
    ...
    $.ec.Base = function() {
        ...
        $().add( this.aInput )
           .add( this.bInput )
           .on("input", function() {
            if (curve.aInput.val().length > 0
             && curve.bInput.val().length > 0) {
                 curve.update();
            }
        });
    ...
    $.ec.reals.PointAddition = function() {
        ...
        $().add( this.pxInput )
           ...
           .add( this.qyInput )
           .on("input", function() {
                if (curve.pxInput.val().length > 0
                 && curve.pyInput.val().length > 0
                 && curve.qxInput.val().length > 0
                 && curve.qyInput.val().length > 0) {
                    curve.update();
                }
        });
    ...
    $.ec.modk.Base = function() {
        ...
        var curve = this;
        this.kInput.on("input", function() {
            if (curve.kInput.val().length > 0) {
                curve.update();
            }
        });
    };
